...
def createValXSSA(ts, val_size, seq_length, scalerssa, ssa):
  ...
  for i in tqdm(range(val_size-seq_length)):
    if (-val_size + (i+seq_length)+1) == 0:
      res = applySSA(ts[:], ssa)
    else:
...
def applySSA(ts, ssa):
    ...
    res = np.array(resD['F1'])
    for key in list(resD.keys())[1:]:
        res += np.array(resD[key])
    return res
...
def splitData(ts, seq_len, TRAIN_SIZE, VAL_SIZE, TEST_SIZE, ssa, val_ssa = False):
  ...
  scalerssa = StandardScaler()

  dTrain = ts[:TRAIN_SIZE]
  dVal = ts[TRAIN_SIZE:(TRAIN_SIZE+VAL_SIZE)]
  dTest = ts[(TRAIN_SIZE+VAL_SIZE):]

  scaler = scaler.fit(np.expand_dims(dTrain, axis=1))
  dTrain = scaler.transform(np.expand_dims(dTrain, axis=1)).squeeze()
  dVal = scaler.transform(np.expand_dims(dVal, axis=1)).squeeze()
  dTest = scaler.transform(np.expand_dims(dTest, axis=1)).squeeze()
...
def getPredsTomorrow(model, seq, groundTruth, scaler, testSize, seq_length, verbose = False):
    ...
    for i in tqdm(range(testSize), disable= not verbose):
      y_test_pred = model(test_seq[None])
      pred = torch.flatten(y_test_pred).item()
      preds.append(pred)
      new_seq = test_seq.numpy().flatten()
